...
import { crmMemberSave } from '@/api/customermanagement/member'
...
export default {
  ...
  methods: {
    ...
    submiteParams(params) {
      ...
      crmRequest(params)
        .then(res => {
          ...
          this.loading = false
          if (this.crmType == 'customer'||this.crmType == 'member') {
    ...
    getSubmiteRequest() {
      ...
      } else if (this.crmType == 'member') {
        return crmMemberSave
    ...
    getTitle() {
      if (this.crmType == 'leads') {
        ...
        return this.action.type == 'update' ? '编辑会员' : '新建会员'
      }else if (this.crmType == 'business') {
